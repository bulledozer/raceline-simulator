...
func cm_add_point(pos:Vector3,_in:Vector3=Vector3.ZERO,_out:Vector3=Vector3.ZERO,idx:int=-1)->void:
	add_point(pos,_in,_out,idx)
	update_control_points()

func cm_set_point_position(idx:int,pos:Vector3)->void:
	set_point_position(idx,pos)
	update_control_points()

func cm_remove_point(idx:int)->void:
	remove_point(idx)
	update_control_points()
